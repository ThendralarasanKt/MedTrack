...
class LocalMcpHttpServer @Inject constructor(
    ...
    private val bearerTokenProvider: McpBearerTokenProvider
) {
    @Volatile
    ...
    fun start() {
        ...
        Log.i(TAG, "MCP server listening on http://$HOST:$PORT$PATH (Authorization Bearer required)")
    }
    ...
    private fun route(request: HttpRequest): HttpResponse {
        ...
        val authReject = McpHttpAuth.rejectStatus(request.headers, bearerTokenProvider.token())
        if (authReject != null) {
            return HttpResponse.text(
                authReject,
                if (authReject == 401) "Unauthorized" else "Forbidden"
            )
        }
